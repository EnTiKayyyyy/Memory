...
//                onReceiveLeaveRoom("");
    ...
    private void onReceiveLogin(String received) {
        // Tách chuỗi nhận được bằng dấu chấm phẩy
        String[] splitted = received.split(";");

        // Kiểm tra xem mảng có đủ các phần tử cần thiết không
        if (splitted.length < 3) {
            System.out.println("Chuỗi nhận được không hợp lệ: " + received);
            return; // Thoát nếu chuỗi không hợp lệ
        }

        // Lấy email / password từ chuỗi đã tách
        ...
        if (result != null && result.split(";")[0].equals("success")) {
            ...
            System.out.println("Đăng nhập không thành công hoặc phản hồi không hợp lệ");
        }

        // Gửi kết quả
        sendData("LOGIN" + ";" + (result != null ? result : "error;null"));
        onReceiveGetListOnline();
    }
